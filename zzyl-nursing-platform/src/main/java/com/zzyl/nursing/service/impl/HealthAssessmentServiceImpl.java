...
import com.zzyl.common.ai.BaiduAIModelInvoker;
...
public class HealthAssessmentServiceImpl extends ServiceImpl<HealthAssessmentMapper, HealthAssessment> implements IHealthAssessmentService {
    ...
    @Autowired
    private BaiduAIModelInvoker baiduAIModelInvoker;
    @Autowired
    ...
    public Long insertHealthAssessment(HealthAssessmentDto dto) {
        ...
        //调用千帆大模型获取数据，返回json格式的数据
        String aiResult = baiduAIModelInvoker.qianfanInvoker(prompt);
        // 解析数据
        HealthReportVo healthReportVo = JSONUtil.toBean(aiResult, HealthReportVo.class);
        // 保存数据
        ...
        healthAssessment.setIdCard(idCard);//身份证号码
        healthAssessment.setBirthDate(IDCardUtils.getBirthDateByIdCard(idCard));// 出生日期
        healthAssessment.setAge(IDCardUtils.getAgeByIdCard(dto.getIdCard()));// 年龄
        healthAssessment.setGender(IDCardUtils.getGenderFromIdCard(dto.getIdCard()));//性别(0:男，1:女)

        //健康评估信息:风险等级和健康指数
        healthAssessment.setRiskLevel(healthReportVo.getHealthAssessment().getRiskLevel());//风险等级
        healthAssessment.setHealthScore(String.valueOf(healthReportVo.getHealthAssessment().getHealthIndex()));//健康指数

        //是否建议入住 健康指数>60 建议入住 （0：建议，1：不建议）
        healthAssessment.setSuggestionForAdmission(healthReportVo.getHealthAssessment().getHealthIndex() > 60 ? 0 : 1);//建议入住

        //推荐护理等级
        healthAssessment.setNursingLevelName(getNursingLevelName(healthReportVo.getHealthAssessment().getHealthIndex()));//推荐护理等级
        ...
        healthAssessment.setDiseaseRisk(JSONUtil.toJsonStr(healthReportVo.getRiskDistribution()));//疾病风险
        healthAssessment.setAbnormalAnalysis(JSONUtil.toJsonStr(healthReportVo.getAbnormalData()));//异常分析
        healthAssessment.setSystemScore(JSONUtil.toJsonStr(healthReportVo.getSystemScore()));//健康系统分值
     ...
     * 根据健康评分获取护理等级名称
     *
     * @param healthScore 健康评分，取值范围为0-100
     * @return 护理等级名称，包括"三级护理等级"、"二级护理等级"、"一级护理等级"、"特级护理等级"或"无"
     * @throws IllegalArgumentException 当健康评分不在0到100范围内时抛出异常
     */
    ...
    private String getPrompt(HealthAssessmentDto dto) {
        //从redis中获取体检报告内容
        String pdfContent = (String) redisTemplate.opsForHash().get(CacheConstants.HEALTH_REPORT, dto.getIdCard());
        if (StringUtils.isEmpty(pdfContent)) {
            throw new ServiceException("体检报告内容不能为空");
        }
